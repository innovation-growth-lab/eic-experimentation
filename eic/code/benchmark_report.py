...
class RadarPlot:
    ...
    def plot(self, highlight: str = None, disclose: bool = False) -> None:
        ...
        for label, angle in zip(labels, angles[:-1]):
            ...
            ax.text(angle, 9*distance, label, size=10, horizontalalignment=ha, verticalalignment="center")
        ...
        ax.legend(loc='lower center', bbox_to_anchor=(0.5, -0.05), ncol=len(self.agencies), handlelength=1)

        plt.tight_layout()
        radar_output_path = os.path.join(PROJECT_DIR, 'eic/outputs/figures', f"radar_plot_{'all' if not highlight else highlight}.png")
    ...
    def plot(self, highlight: str = None, disclose: bool = False):
        ...
        for i, cat in enumerate(self.categories):
            ...
            for ax, sub_cat in zip(axs, sub_categories):
                scores = sub_df[sub_df['Sub-category'] == sub_cat][self.agencies].values[0]
                ...
                ax.bar(self.agencies, scores, color=[self.colors[agency] if agency == highlight else 'gray' for agency in self.agencies], zorder=2)
            ...
            # plot and save
            plt.tight_layout()
            histogram_output_path = os.path.join(PROJECT_DIR, 'eic/outputs/figures', f"histogram_plot_{'all' if not highlight else highlight}_{i}.png")
...
def generate_colored_table(df: pd.DataFrame):
    ...
    # Display the table
    ax.table(cellText=df.values, colLabels=df.columns, cellColours=colors.values, loc='center')
...
def preprocess_data(data_path: str) -> pd.DataFrame:
    ...
    orig_rows = df.loc[(df["Sub-category"]=="Extensive margin")|(df["Sub-category"]=="Intensive margin")]
    new_row = orig_rows.sum(numeric_only=True)
    new_row["Sub-category"] = "Data Use"
    new_row["Category"] = "Collection & Integration"
    df = pd.concat([df, new_row.to_frame().T.set_index(pd.Index([5]))], ignore_index=False).sort_index()
